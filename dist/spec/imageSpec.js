...
const imageProcessing_1 = __importDefault(
  require('../src/utilities/imageProcessing')
);
...
describe('processImage utility', () => {
  ...
  it('throws on invalid dimensions', async () => {
    await expectAsync(
      (0, imageProcessing_1.default)('fjord', 0, 200)
    ).toBeRejected();
  });
  it('throws when source image not found', async () => {
    await expectAsync(
      (0, imageProcessing_1.default)('missing', 100, 100)
    ).toBeRejected();
  });
});
