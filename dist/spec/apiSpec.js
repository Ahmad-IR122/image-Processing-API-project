...
describe('API /api/images', () => {
    ...
    it('returns 400 for invalid dimensions', async () => {
        const res = await request.get('/api/images?imageName=fjord&width=-1&height=200');
        expect(res.status).toBe(400);
    });
    it('returns 404 for non-existent image', async () => {
        const res = await request.get('/api/images?imageName=doesnotexist&width=200&height=200');
        expect(res.status).toBe(404);
    });
    it('returns 200 and serves an image for valid request', async () => {
        const res = await request.get('/api/images?imageName=fjord&width=200&height=200');
        expect(res.status).toBe(200);
        expect(res.headers['content-type']).toContain('image');
    });
    it('uses cache on second request (same dimensions)', async () => {
        const res1 = await request.get('/api/images?imageName=fjord&width=220&height=180');
